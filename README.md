...
- ⚡ **服务未运行时自动拉起**：默认执行 `npx --yes @deepseek-ai/dsh web`（全静默、无终端窗口），
  初次启动/重启电脑后双击应用即可直接用，无需先手动开服务
- ⏱️ 借鉴 opencode 桌面端：启动就绪等待 + 120s 超时看门狗 + 失败自动重试（15s 冷却）
- 🔐 给拉起的服务注入 `--use-system-ca`（内网/代理证书环境）与环回地址免代理
- 🔁 服务离线时显示极简加载页（黑鲸 logo 缓慢变色），每 3 秒自动探测，上线后自动进入
...
- **自动拉起**：检测到 DSH 服务未运行且「自动启动」开启时，静默执行 `npx --yes @deepseek-ai/dsh web`，服务就绪后自动进入
- **开机自启**：菜单「服务 → 开机自动启动 DSH Desktop」开启后，Windows 登录时自动启动应用（配合自动拉起，重启后双击即用）
